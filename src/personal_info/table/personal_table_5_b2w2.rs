...
use crate::personal_info::PersonalInfo5B2W2;
use std::ops::Index;

pub struct PersonalTable5B2BW<'a> {
    table: Vec<PersonalInfo5B2W2<'a>>,
}

impl<'a> PersonalTable5B2BW<'a> {
    const SIZE: usize = PersonalInfo5B2W2::SIZE;

    pub fn new(data: &'a [u8]) -> PersonalTable5B2BW<'a> {
        let count = data.len() / Self::SIZE;
        let mut table = Vec::with_capacity(count);
        let mut ofs = 0;
        for _ in 0..count {
            let slice = &data[ofs..(ofs + Self::SIZE)];
            table.push(PersonalInfo5B2W2::new(slice));
            ofs += Self::SIZE;
        }
        Self { table }
    }
...
impl<'a> Index<usize> for PersonalTable5B2BW<'a> {
    type Output = PersonalInfo5B2W2<'a>;
...
impl<'a> Index<(u16, u8)> for PersonalTable5B2BW<'a> {
    type Output = PersonalInfo5B2W2<'a>;
...
impl<'a> PersonalTable for PersonalTable5B2BW<'a> {
    type PersonalInfoInner = PersonalInfo5B2W2<'a>;
