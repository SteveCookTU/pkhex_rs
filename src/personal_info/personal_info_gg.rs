...
use crate::{get_bits, PersonalInfo};

pub struct PersonalInfoGG {
    child: PersonalInfoSM,
    tmhm: Vec<bool>,
...
impl PersonalInfoGG {
    ...
    pub fn get_special_z_item(&self) -> usize {
        self.child.get_special_z_item()
    }

    pub fn set_special_z_item(&mut self, special_z_item: usize) {
        self.child.set_special_z_item(special_z_item);
    }

    pub fn get_special_z_base_move(&self) -> usize {
        self.child.get_special_z_base_move()
    }

    pub fn set_special_z_base_move(&mut self, special_z_base_move: usize) {
        self.child.set_special_z_base_move(special_z_base_move);
    }

    pub fn get_special_z_move(&self) -> usize {
        self.child.get_special_z_move()
    }

    pub fn set_special_z_move(&mut self, special_z_move: usize) {
        self.child.set_special_z_move(special_z_move);
    }

    pub fn get_local_variant(&self) -> bool {
        self.child.get_local_variant()
    }

    pub fn set_local_variant(&mut self, local_variant: bool) {
        self.child.set_local_variant(local_variant);
    }

    pub fn get_go_species(&self) -> usize {
        u16::from_le_bytes(self.child.get_data()[0x48..0x4A].try_into().unwrap()) as usize
    }

    pub fn set_go_species(&mut self, go_species: usize) {
        let go_species_bytes = (go_species as u16).to_le_bytes();
        self.child
            .child
            .child
            .data
            .splice(0x48..0x4A, go_species_bytes);
    }
}

impl PersonalInfo for PersonalInfoGG {
    fn new(data: Vec<u8>) -> Self {
        Self {
            tmhm: get_bits(&data[0x28..0x30]),
            type_tutors: get_bits(&data[0x38..0x39]),
            child: PersonalInfoSM::new_child(data),
        }
    }
