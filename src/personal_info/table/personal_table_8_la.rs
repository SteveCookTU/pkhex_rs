...
use crate::personal_info::PersonalInfo8LA;
use std::ops::Index;

pub struct PersonalTable8LA<'a> {
    table: Vec<PersonalInfo8LA<'a>>,
}

impl<'a> PersonalTable8LA<'a> {
    const SIZE: usize = PersonalInfo8LA::SIZE;

    pub fn new(data: &'a [u8]) -> PersonalTable8LA<'a> {
        let count = data.len() / Self::SIZE;
        let mut table = Vec::with_capacity(count);
        let mut ofs = 0;
        for _ in 0..count {
            let slice = &data[ofs..(ofs + Self::SIZE)];
            table.push(PersonalInfo8LA::new(slice));
            ofs += Self::SIZE;
        }
        Self { table }
    }
...
impl<'a> Index<usize> for PersonalTable8LA<'a> {
    type Output = PersonalInfo8LA<'a>;
...
impl<'a> Index<(u16, u8)> for PersonalTable8LA<'a> {
    type Output = PersonalInfo8LA<'a>;
...
impl<'a> PersonalTable for PersonalTable8LA<'a> {
    type PersonalInfoInner = PersonalInfo8LA<'a>;
