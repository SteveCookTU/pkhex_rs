...
use crate::pkm::shared::{EntityContext, ObedienceLevel, ObedienceLevelReadOnly};
...
use crate::pkm::traits::templates::{
    ContestStatsReadOnly, MemoryHTReadOnly, MemoryOTReadOnly, NatureReadOnly, ScaledSizeReadOnly,
    TeraTypeReadOnly,
};
use crate::pkm::traits::tera_type::TeraType;
use crate::pkm::traits::{
    tera_type, BattleVersion, ContestStats, Favorite, HandlerLanguage, HomeTrack, HyperTrain,
    MemoryHT, MemoryOT, MoveReset, Nature, SanityChecksum, ScaledSize, ScaledSize3, TechRecord,
    TrainerID, TrainerIDPKM, TrainerMemories,
};
...
use crate::ribbons::{
    ...
    RibbonSetCommon6, RibbonSetCommon7, RibbonSetCommon8, RibbonSetCommon9, RibbonSetEvent3,
    RibbonSetEvent4, RibbonSetMark8, RibbonSetMark9, RibbonSetMemory6,
};
use crate::{flag_util, legality, personal_info};
use no_std_io::{Reader, Writer};

static UNUSED: [u16; 78] = [
    0x17, 0x1A, 0x1B, 0x23, 0x33, 0x3E, 0x3F, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53,
    0x54, 0x55, 0x56, 0x57, 0x90, 0x91, 0x92, 0x93, // Status condition
    0x96, 0x97, 0x98, 0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5,
    0xA6, 0xA7, 0xC5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, 0xDC, 0xDD, 0xDE, 0xDF, 0xE0, 0xE1, 0xE2,
    0xE3, 0xE4, 0xE5, 0xE6, 0xE7, 0xE8, 0xE9, 0xEA, 0xEB, 0xEC, 0xED, 0xEE, 0xEF, 0xF0, 0xF1, 0xF2,
    0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0x115,
];
...
impl PK9 {
    pub(crate) const COUNT_RECORD: usize = 200;
    const RECORD_START: usize = 0x12F;
    ...
    fn is_unhatched_egg(&self) -> bool {
        self.version() == 0 && self.is_egg()
    }

    pub fn clear_move_record_flags(&mut self) {
        self.data[PK9::RECORD_START..(PK9::RECORD_START + PK9::RECORD_LENGTH)].fill(0)
    }
...
impl From<Vec<u8>> for PK9 {
    fn from(value: Vec<u8>) -> Self {
        let mut pk9 = Self { data: value };
        pk9.set_tera_type_override(MoveType::from(19));
        pk9
    }
}

impl SanityChecksum for PK9 {
    fn sanity(&self) -> u16 {
...
impl TeraTypeReadOnly for PK9 {
    fn tera_type(&self) -> MoveType {
        tera_type::get_tera_type_from_param(
...
impl TeraType for PK9 {
    fn tera_type_original(&self) -> MoveType {
        MoveType::from(self.data.default_read_le::<i8>(0x94))
    }

    fn set_tera_type_original(&mut self, t: MoveType) {
        self.data.checked_write_le(0x94, &(t as i8));
    }

    fn tera_type_override(&self) -> MoveType {
        MoveType::from(self.data.default_read_le::<i8>(0x95))
    }

    fn set_tera_type_override(&mut self, t: MoveType) {
        self.data.checked_write_le(0x95, &(t as i8));
    }
}

impl MoveReset for PK9 {
    fn reset_moves(&mut self) {
        let learn_sets = &legality::legality_core::LEVEL_UP_SV;
        let table = &personal_info::SV;
...
impl TechRecord for PK9 {
    fn tech_record_permit_flags(&self) -> Vec<bool> {
        self.personal_info().tmhm().to_vec()
    }

    fn tech_record_permit_indexes(&self) -> Vec<u16> {
        legality::learn_source::TM_SV.to_vec()
    }

    fn record_count_total(&self) -> u16 {
        PK9::COUNT_RECORD as u16
    }

    fn record_count_used(&self) -> u16 {
        PersonalInfo9SV::COUNT_TM
    }

    fn get_move_record_flag(&self, index: u16) -> bool {
        if index > PK9::COUNT_RECORD as u16 {
            return false;
        }
        let ofs = (index >> 3) as usize;
        flag_util::get_flag(&self.data, PK9::RECORD_START + ofs, (index & 7) as usize)
    }
    ...
    fn get_move_record_flag_any(&self) -> bool {
        self.data
            .iter()
            .skip(PK9::RECORD_START)
            .take(PK9::RECORD_LENGTH)
            .any(|&i| i != 0)
    }
}

impl ObedienceLevelReadOnly for PK9 {
    fn obedience_level(&self) -> u8 {
        self.data.default_read_le(0x11F)
    }
}

impl ObedienceLevel for PK9 {
    fn set_obedience_level(&mut self, level: u8) {
        self.data.checked_write_le(0x11F, &level);
    }
}

impl RibbonSetEvent3 for PK9 {
...
impl RibbonSetAffixed for PK9 {
    fn affixed_ribbon(&self) -> Option<u8> {
        let data = self.data.default_read_le::<i8>(0xD4);
        if data < 0 {
            None
        } else {
            Some(data as u8)
...
impl RibbonSetCommon9 for PK9 {
    ...
    fn set_ribbon_once_in_a_lifetime(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 2, val)
    }
}

impl ContestStatsReadOnly for PK9 {
    fn cnt_cool(&self) -> u8 {
...
impl RibbonSetMark9 for PK9 {
    ...
    fn set_ribbon_mark_mini(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x44, 6, val)
    }
    ...
    fn set_ribbon_mark_item_finder(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x44, 7, val)
    }
    ...
    fn set_ribbon_mark_partner(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 0, val)
    }
    ...
    fn set_ribbon_mark_gourmand(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 1, val)
    }
    ...
    fn set_ribbon_mark_alpha(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 3, val)
    }
    ...
    fn set_ribbon_mark_mightiest(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 4, val)
    }
    ...
    fn set_ribbon_mark_titan(&mut self, val: bool) {
        flag_util::set_flag(&mut self.data, 0x45, 5, val)
    }
}

impl HyperTrain for PK9 {
...
impl ScaledSizeReadOnly for PK9 {
    fn weight_scalar(&self) -> u8 {
        self.data.default_read_le(0x49)
    }
...
impl ScaledSize for PK9 {
    fn set_weight_scalar(&mut self, scalar: u8) {
        self.data.checked_write_le(0x49, &scalar);
    }

    fn set_height_scalar(&mut self, scalar: u8) {
        self.data.checked_write_le(0x48, &scalar);
    }
}

impl ScaledSize3 for PK9 {
    fn scale(&self) -> u8 {
        self.data.default_read_le(0x4A)
    }

    fn set_scale(&mut self, scale: u8) {
        self.data.checked_write_le(0x4A, &scale);
    }
}

impl Favorite for PK9 {
...
impl FormArgument for PK9 {
    fn form_argument(&self) -> u32 {
        self.data.default_read_le(0xD0)
    }

    fn set_form_argument(&mut self, arg: u32) {
        self.data.checked_write_le(0xD0, &arg);
    }
...
impl HomeTrack for PK9 {
    fn tracker(&self) -> u64 {
        self.data.default_read_le(0x127)
    }

    fn set_tracker(&mut self, tracker: u64) {
        self.data.checked_write_le(0x127, &tracker);
    }
}

impl BattleVersion for PK9 {
    fn battle_version(&self) -> u8 {
        self.data.default_read_le(0xCF)
    }

    fn set_battle_version(&mut self, version: u8) {
        self.data.checked_write_le(0xCF, &version);
    }
}

impl MemoryOT for PK9 {
...
impl TrainerMemories for PK9 {}

impl SpeciesForm for PK9 {
    fn species(&self) -> u16 {
...
impl TrainerIDPKM for PK9 {}

impl TrainerID for PK9 {
    fn tid(&self) -> u16 {
...
impl LangNick for PK9 {
    ...
    fn language(&self) -> u8 {
        self.data.default_read_le(0xD5)
    }
}

impl GameValueLimit for PK9 {
    fn max_species_id(&self) -> u16 {
        legality::tables9::MAX_SPECIES_ID_9
    }

    fn max_move_id(&self) -> u16 {
        legality::tables9::MAX_MOVE_ID_9
    }

    fn max_item_id(&self) -> u16 {
        legality::tables9::MAX_ITEM_ID_9
    }

    fn max_ability_id(&self) -> u16 {
        legality::tables9::MAX_ABILITY_ID_9
    }

    fn max_ball_id(&self) -> u8 {
        legality::tables9::MAX_BALL_ID_9
    }

    fn max_game_id(&self) -> u8 {
        legality::tables9::MAX_GAME_ID_9
    }
...
impl Pkm for PK9 {
    type PersonalInfoOutput = PersonalInfo9SV<'static>;

    fn size_party(&self) -> usize {
        poke_crypto::SIZE_9PARTY
    }

    fn size_stored(&self) -> usize {
        poke_crypto::SIZE_9STORED
    }
    ...
    fn personal_info(&self) -> &Self::PersonalInfoOutput {
        personal_info::SV.get_form_entry(self.species(), self.form())
    }
    ...
    fn nickname_trash(&self) -> &[u8] {
        &self.data[0x58..(0x58 + 26)]
    }

    fn nickname_trash_mut(&mut self) -> &mut [u8] {
        &mut self.data[0x58..(0x58 + 26)]
    }

    fn ot_trash(&self) -> &[u8] {
        &self.data[0xF8..(0xF8 + 26)]
    }

    fn ot_trash_mut(&mut self) -> &mut [u8] {
        &mut self.data[0xF8..(0xF8 + 26)]
    }

    fn ht_trash(&self) -> &[u8] {
        &self.data[0xA8..(0xA8 + 26)]
    }

    fn ht_trash_mut(&mut self) -> &mut [u8] {
        &mut self.data[0xA8..(0xA8 + 26)]
    }

    fn encrypt(&mut self) -> Vec<u8> {
        self.refresh_checksum();
        poke_crypto::encrypt_array_9(&self.data)
    }
    ...
    fn gender(&self) -> u8 {
        (self.data.default_read_le::<u8>(0x22) >> 1) & 0x3
    }

    fn set_gender(&mut self, gender: u8) {
        let gender = (self.data.default_read_le::<u8>(0x22) & 0xF9) | (gender << 1);
        self.data.checked_write_le(0x22, &gender);
    }

    fn stat_nature(&self) -> u8 {
        self.data.default_read_le(0x21)
    }

    fn set_stat_nature(&mut self, nature: u8) {
        self.data.checked_write_le(0x21, &nature);
    }
    ...
    fn set_status_condition(&mut self, status: u32) {
        self.data.checked_write_le(0x90, &status);
    }
    ...
    fn version(&self) -> u8 {
        self.data.default_read_le(0xCE)
    }

    fn set_version(&mut self, version: u8) {
        self.data.checked_write_le(0xCE, &version);
    }
    ...
    fn set_language(&mut self, lang: u8) {
        self.data.checked_write_le(0xD5, &lang);
    }
    ...
    fn is_untraded(&self) -> bool {
        self.data.default_read_le::<u8>(0xA8) == 0
            ...
            && (self.is_unhatched_egg() || self.format().unwrap_or_default() == self.generation())
    }
